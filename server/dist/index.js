...
import { addParticipantSchema, createEventSchema, createPlayerSchema, createSeasonSchema, updateEventSchema, updateParticipantSchema, updatePlayerSchema, updateSeasonSchema } from "./validators.js";
import { canLockEvent, computeRanking } from "./ranking.js";
import { nextId, readStore, writeStore } from "./storage.js";
const app = express();
...
function validatePrizeRanks(prizeRanks) {
...
async function getParticipants(eventId) {
    const store = await readStore();
    const participants = store.eventParticipants.filter((participant) => participant.eventId === eventId);
...
app.get("/api/players", async (req, res) => {
    ...
    const players = store.players
        ...
        .filter((player) => normalizedQuery === "" ? true : player.name.toLowerCase().includes(normalizedQuery))
...
app.post("/api/players", async (req, res) => {
    ...
    try {
        const player = await writeStore((store) => {
            const exists = store.players.some((entry) => normalizeName(entry.name) === normalizeName(name));
            if (exists) {
    ...
    try {
        await writeStore((store) => {
            ...
            if (parsed.data.name !== undefined) {
                const name = parsed.data.name.trim();
                const exists = store.players.some((entry) => entry.id !== playerId && normalizeName(entry.name) === normalizeName(name));
                if (exists) {
                    throw new Error(DUPLICATE_PLAYER);
                }
                player.name = name;
...
app.patch("/api/events/:id", async (req, res) => {
    ...
    let nextPrizeRanks = null;
    if (parsed.data.prizeRank1 !== undefined ||
        parsed.data.prizeRank2 !== undefined ||
        parsed.data.prizeRank3 !== undefined) {
        nextPrizeRanks = [
...
app.post("/api/events/:id/participants", async (req, res) => {
    ...
    try {
        const participant = await writeStore((store) => {
            const participants = store.eventParticipants.filter((entry) => entry.eventId === eventId);
            ...
            const exists = store.eventParticipants.some((entry) => entry.eventId === eventId && entry.playerId === parsed.data.playerId);
            if (exists) {
...
app.patch("/api/events/:eventId/participants/:participantId", async (req, res) => {
    ...
    await writeStore((store) => {
        const participant = store.eventParticipants.find((entry) => entry.id === participantId);
        if (!participant) {
            return false;
        }
        let updated = false;
...
app.get("/api/seasons/:id/ranking", async (req, res) => {
    ...
    const blockingOpenEvents = relevant.filter((event) => event.status !== "LOCKED" && event.eventDate <= today);
    ...
    const lockedEventIds = new Set(relevant.filter((event) => event.status === "LOCKED").map((event) => event.id));
    const rows = store.eventParticipants
        .filter((participant) => lockedEventIds.has(participant.eventId))
        .filter((participant) => participant.pointsR1 !== null &&
        ...
        return {
            player_id: participant.playerId,
            player_name: player.name,
            total_points: (participant.pointsR1 ?? 0) +
